...
```text
你现在接手 `/root/swe` 里的 SWE-Trace / SWE-Code-Agent 项目。请先阅读：

- `/root/swe/AGENTS.md`（如果存在）
- `/root/swe/docs/HANDOFF.md`
- `/root/swe/README.md`
- `/root/swe/reports/progress.html`
...
- Docker preflight 脚本
- progress HTML 报告页
- pytest 上次为 19 passed

当前主要阻塞：

- 原机器没有 Docker binary 和 `/var/run/docker.sock`，无法运行 SWE-bench testbed。
- 现在目标机器应该是 3090，请优先确认 Docker 是否可用。

请你不要重新设计项目，直接继续执行下一步：

1. 在 `/root/swe` 恢复/创建 `.venv`。
2. 安装依赖并运行 `/root/swe/.venv/bin/python -m pytest -q`。
3. 运行 `./scripts/check_docker.sh`。
4. 如果 Docker 可用，运行 `./scripts/download_swebench_lite.sh`。
5. 然后运行：
   `SWETRACE_MINI_SUBSET=/root/swe/cache/swebench_lite SWETRACE_MINI_INSTANCE=sqlfluff__sqlfluff-1625 ./scripts/run_mini_smoke.sh`
...
8. 构建真实 SFT/debug/reward 数据，补人工标注 CLI。
9. 每次项目推进都更新 `/root/swe/reports/progress.html`，并保持 GitHub 同步到 `https://github.com/yiyu0716/swe-code-agent.git`。
